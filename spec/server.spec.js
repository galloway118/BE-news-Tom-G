...
describe('/api', () => {
  ...
  describe('/api/users', () => {
    it('GET:200: returns users in array of objects', () => {
      return request(server)
        .get('/api/users')
        .expect(200)
        .then(users => {
          expect(users.body).to.have.keys('users');
          expect(users.body.users[0]).to.have.keys(
            'username',
            'name',
            'avatar_url'
          );
    ...
    describe('/api/article/:article_id', () => {
      ...
      it('ERROR - GET:404 return message when article id not in database', () => {
        return request(server)
          ...
          .expect(404)
          .then(error => expect(error.body.msg).to.equal('article not found'));
      });
      ...
      it('ERROR - GET:400 return message when article id not valid', () => {
        return request(server)
          ...
          .expect(400)
          .then(error => expect(error.body.msg).to.equal('Invalid input type'));
      });
      ...
      describe('/api/articles/:article_id/comments', () => {
        ...
        it('Post: 201 ', () => {
          return request(server)
            ...
            .expect(201)
            .then(comment => {
              expect(comment.body.comment).to.have.keys(
                'comment_id',
                'votes',
                'created_at',
                'author',
          ...
          return request(server)
            ...
            .then(err => {
              expect(err.body.msg).to.equal(
                'Post violates non null constraints'
              );
            });
        });
        it('ERROR - POST: 404 when wrong value type passed into database', () => {
          return request(server)
            .post('/api/articles/1/comments')
            .send({ username: 'yabadabadooo', body: 'ssdkdf' })
            .expect(404)
            .then(err => {
        ...
        it('ERROR - GET 400 when order is invalid', () => {
          return request(server)
            .get(
              '/api/articles/1/comments?sort_by=created_at&order=this_is_not_correct'
            )
            .expect(400)
            .then(err => {
              expect(err.body.msg).to.equal('Invalid order value');
